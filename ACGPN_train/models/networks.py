...
def define_UnetMask(opt, input_nc, gpu_ids=[]):
    netG = UnetMask(opt, input_nc,output_nc=4)
    netG.cuda(gpu_ids[0])
    netG.apply(weights_init)
    return netG
...
class UnetMask(nn.Module):
    ...
    def forward(self, input, refer, mask, lm_c_map, cloth_rep):
        ...
        # input = cloth image
        # cloth_rep = cloth image + landmarks
...
class Unet(nn.Module):
    ...
    def forward(self, input, refer, mask, lm_c_map, cloth_rep):
        input, warped_mask,rx,ry,cx,cy, warped_cloth_lm = self.stn(input, torch.cat([mask, refer, input], 1), mask, lm_c_map)
        ## input, warped_mask, warped_cloth_lm, rx, ry, cx, cy = self.stn(input, torch.cat([mask, cloth_rep, input], 1), mask,lm_c_map)
        #ipdb.set_trace()
        conv1 = self.conv1(torch.cat([refer.detach(), input.detach()], 1))
        pool1 = self.pool1(conv1)
        ...
        up9 = self.up9(conv8)
        conv9 = self.conv9(torch.cat([conv1, up9], 1))
        return conv9, input, warped_mask,rx,ry,cx,cy, warped_cloth_lm
...
class CNN(nn.Module):
    def __init__(self, opt, num_output, input_nc=5, ngf=8, n_layers=5, norm_layer=nn.InstanceNorm2d, use_dropout=False):
...
class BoundedGridLocNet(nn.Module):
    ...
    def __init__(self, opt, grid_height, grid_width, target_control_points):
        super(BoundedGridLocNet, self).__init__()
        self.opt = opt
        self.cnn = CNN(self.opt, grid_height * grid_width * 2)
...
class STNNet(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.loc_net = GridLocNet(self.opt, grid_size_h, grid_size_w, target_control_points)
    ...
    def get_col(self,coor,num):
        for i in range(num):
            ...
            for j in range(num - 1):
                ...
                sum += second_dif
            print(sum)
    def forward(self, x, reference, mask, lm_c_map):

        # reference = pre cloth mask + warp cloth mask + cloth_rep (cloth_image + flm)
        ...
        # landmarks warp
        warped_cloth_lm = grid_sample(lm_c_map, grid)

        return transformed_x, warped_mask, warped_cloth_lm, rx,ry,cx,cy,rg,cg
