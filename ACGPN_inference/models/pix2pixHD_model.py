...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if not self.isTrain or opt.continue_train or opt.load_pretrain:
            ...
            pretrained_path = self.opt.checkpoints_dir + '/' + self.opt.name
    ...
    def forward(self, label, pre_clothes_mask, img_fore, clothes_mask, clothes, all_clothes_label, real_image, pose,grid,cloth_rep, mesh, dense, mask_fore):
        ...
        if self.opt.denseplus or self.opt.densestack:
            G1_in = torch.cat([pre_clothes_mask, cloth_rep, all_clothes_label, dense, pose, self.gen_noise(shape)], dim=1)
